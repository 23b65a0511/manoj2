...
#     cur.execute(
#         "INSERT INTO history VALUES (?,?,?,?,?,?)",
#         (name, age, gender, label, prob, datetime.now())
#     )
...
# # ================= DATABASE PATHS =================
# DB_USERS = os.path.join(app.root_path, "users.db")
# DB_HISTORY = os.path.join(app.root_path, "history.db")
...
#     arrow_end = ((x1 + x2)//2, (y1 + y2)//2)
#     cv2.arrowedLine(img_color, arrow_start, arrow_end, (255,0,0), 2)
#     cv2.putText(img_color, "Detected ROI", (arrow_start[0]-50, arrow_start[1]-10),
...
def forgot_password():
    if request.method == 'POST':
        ...
        conn.close()
        if result:
            return render_template("forgot_password.html", message=f"Your password is: {result[0]}")
        else:
